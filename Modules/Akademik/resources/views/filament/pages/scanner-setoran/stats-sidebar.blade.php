<aside class="space-y-4">
    ...
    <div class="qr-stat-row">
        ...
        <div class="qr-stat-card qr-stat-total">
            <div class="relative z-10">
                <p class="qr-stat-num" x-text="$wire.todayStats.total ?? 0"></p>
                <p class="qr-stat-lbl">Setoran Hari Ini</p>
            </div>
            <svg class="qr-stat-icon" fill="currentColor" viewBox="0 0 24 24">
        ...
        <div class="qr-stat-card qr-stat-a">
            <div class="relative z-10">
                <p class="qr-stat-num" x-text="$wire.todayStats.countA ?? 0"></p>
                <p class="qr-stat-lbl">Sangat Lancar (A)</p>
            </div>
            <svg class="qr-stat-icon" fill="currentColor" viewBox="0 0 24 24">
    ...
    <div class="qr-card">
        ...
        <div class="qr-history-wrap" id="history-list">
            ...
            <template x-for="scan in $wire.recentScans" :key="scan.id">
                <div class="qr-history-item">
                    ...
                    <div class="text-right flex-shrink-0 flex flex-col items-end gap-1">
                        <p class="text-[10px] text-gray-400 font-bold" x-text="scan.time"></p>
                        <span class="qr-grade-badge"
                              :class="{
                                  'qr-grade-A': scan.grade === 'A',
                                  'qr-grade-B': scan.grade === 'B',
                                  'qr-grade-C': scan.grade === 'C'
                              }"
                              x-text="scan.grade">
                        </span>
                    </div>
            ...
            <template x-if="!$wire.recentScans || $wire.recentScans.length === 0">
                <div class="py-10 text-center">
                    ...
                    <p class="text-xs font-bold text-gray-400">Belum ada setoran hari ini</p>
                    <p class="text-[10px] text-gray-300 mt-0.5">Scan QR Code siswa untuk memulai</p>
                </div>
            </template>
    ...
    {{-- ===== VOLUME ===== --}}
    <div class="qr-card">
        <div class="qr-volume-box flex items-center justify-between gap-3" style="padding: 0.5rem 1rem;">
            <p class="text-[10px] font-black text-gray-400 uppercase tracking-widest flex items-center gap-1.5 mb-0">
                <svg x-show="$wire.volume > 0" class="w-3 h-3 text-gray-400 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                          d="M19.114 5.636a9 9 0 010 12.728M16.463 8.288a5.25 5.25 0 010 7.424M6.75 8.25l4.72-4.72a.75.75 0 011.28.53v15.88a.75.75 0 01-1.28.53l-4.72-4.72H4.51c-.88 0-1.704-.507-1.938-1.354A9.01 9.01 0 012.25 12c0-.83.112-1.633.322-2.396C2.806 8.756 3.63 8.25 4.51 8.25H6.75z"/>
                </svg>
                <svg x-show="$wire.volume == 0" x-cloak class="w-3 h-3 text-gray-300 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <div class="flex items-center gap-2 flex-1 justify-end max-w-[120px]">
                <input type="range" wire:model.live="volume" min="0" max="100"
                       class="flex-1 h-1 rounded-full accent-blue-600 cursor-pointer">
                <span class="text-[10px] font-black text-gray-600 dark:text-gray-400 w-6 text-right tabular-nums"
    ...
    {{-- ===== TIPS BOX ===== --}}
    <div class="qr-tips m-0" style="margin-top: 0;">
        <div class="flex items-start gap-3">
            <div class="p-1.5 bg-blue-600 rounded-lg flex-shrink-0 mt-0.5">
            ...
            <div>
                <p class="text-xs font-black text-blue-900 dark:text-blue-300 mb-1">Cara Menggunakan Scanner</p>
                <ul class="text-[11px] text-blue-700 dark:text-blue-400 space-y-1 list-disc ml-3">
